...
### Responsive launcher and Instructions

The main tabs use shared native layout controls in `ResponsiveLayout.vb`. Wide Launcher and Graphics pages place related groups in two columns; below 1060 logical content pixels they stack. Field rows put labels above sliders/dropdowns below 590 logical pixels, while checkbox rows remain inline. Controls uses stacked action bindings below 650 pixels. The LAN browser switches between Details and Tile views, retaining host/address/player/status information. Content is capped at 1360 logical pixels on very wide windows. Each tab scrolls vertically; the footer reflows independently and remains visible. DPI-aware bounds reuse the original control instances, retaining unsaved values, focus and in-progress capture.

Help has separate About and Instructions tabs. Instructions are embedded offline and use a topic sidebar on wide windows or a dropdown below 720 logical pixels. About retains version, developer and release-update controls. No update network call is needed to read instructions.

Offscreen native UI checks cover all five main tabs at 1440×880, 900×1040, 620×1200 and 560×540 logical client sizes, in light and dark themes. They check horizontal overflow, field bounds, footer reachability, retained edits/capture and Help topic reflow. Mixed-monitor DPI changes and hands-on resizing remain acceptance checks.
...
Desktop mode recovers pending VR changes first, then skips runtime validation/preflight, VR input polling and VR graphics/camera preparation. Desktop VSync and optional borderless preparation are described above. Normal menu launch passes `DIRT2VR_ACTIVE=0` after removing inherited mod flags and does not deploy a proxy. Desktop practice requires `graphics_card/directx@forcedx9=false`, deploys the recognized proxy and passes `DIRT2VR_DESKTOP_PRACTICE=1` with the direct-practice flag. The proxy takes an early path that applies the guarded human-control byte only, before MinHook, render hooks, hotkeys or OpenXR initialization. When logging is enabled, desktop session logs therefore contain only compatibility/control diagnostics.

Version 4 recovery journals own only the desktop practice config, with no game-asset entries; the separate graphics journal records desktop VSync and, when enabled, borderless overrides. They retain the short-path hash/ownership and existing-file conflict checks. Versions 1–3 continue to recover as before. Desktop and VR sessions share the mutex and process-wait routine, including cleanup on launch failure and game exit.
