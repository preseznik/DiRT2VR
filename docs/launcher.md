...
A desktop focus trace showed DiRT 2 replacing its first foreground HWND with a second visible HWND, leaving Explorer in the foreground; the launcher stayed minimized throughout. The session watches only the selected installation's game process for this startup transition and requests foreground for the replacement window once, only if the first window held foreground. Any intervening user input or 30-second timeout cancels this handoff. There is no recurring focus enforcement or synthetic input, and normal Windows activation restrictions apply. Automated state-transition tests pass; the follow-up game check is pending. See [SetForegroundWindow](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-setforegroundwindow) and [AllowSetForegroundWindow](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-allowsetforegroundwindow).
