# DiRT2VR development and investigation

This guide preserves the technical material previously in the root README. For setup and play instructions, see the [end-user README](../README.md); for notable changes, see the [changelog](../CHANGELOG.md). Run the commands below from the repository root.
...
## Recovering settings after an interrupted run

Normal exit restores these files automatically. If the launcher was terminated before restoration, close all DiRT 2 processes first. Find the `artifacts/trace-*` folder belonging to that interrupted run; use its original backups, not the `settings-applied.xml` or modified camera/effects files.

| Backup in that run's folder | Restore to |
|---|---|
| `settings-original.xml` | `Documents/My Games/DiRT2/hardwaresettings/hardware_settings_config.xml` in your Windows Documents folder |
| `effects-original.bin` | `artifacts/game/postprocess/effects.xml` |
| `cameras-original.bin` | `artifacts/game/cars/sti/cameras.xml` |

Copy the backup bytes to the destination filenames shown above. Only restore backups that exist for that run. Do not copy these files into the original Steam installation. Save/profile files are separate and should not be replaced.
