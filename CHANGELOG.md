...
- Experimental Race mode beside Direct practice, with 1–7 AI opponents using the selected car, for desktop and VR launch. Solo practice remains unchanged. Retains the repeating direct-start session and Continue-only pause menu; difficulty, lap count and normal results flow are not exposed. Launcher/configuration/recovery tests pass; opponent driving and crowded-grid VR acceptance remain pending.
