...
- Configurable 1–20 circuit laps for Direct practice and Race, using a guarded in-memory override of the direct-start lap default. Point-to-point stages remain one run, identified from the installed game's route metadata. Original executable and database files stay untouched; gameplay acceptance is pending.
